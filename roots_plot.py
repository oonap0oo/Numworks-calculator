...
def main_loop():
    print("\nRoots of function:")
    print("------------------")
    fun, xstart, xstop = get_input() # get user input
    results = incremental(fun, (xstart, xstop), N) # search for zero crossings, results is list of lists [xl, xu]
    if len(results)>0:
        ...
        plotfun(fun, xstart, xstop, roots, resid)
    else:
        print("No roots found in given interval")


def plotfun(fun, xl, xu, roots, resid):
    xx = np.linspace(xl, xu, 50)
    yy = np.zeros(50)
    for k in range(50):
        yy[k], s = eval_fun(fun, xx[k])
    plt.plot(xx,yy,color="blue")
    plt.scatter(roots,resid,color="red")
    plt.show()
